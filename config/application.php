 ...
 * Default: production
 */
define('WP_ENV', env('WP_ENV') ?: 'prd');

/**
 * Infer WP_ENVIRONMENT_TYPE based on WP_ENV
 */
if (!env('WP_ENVIRONMENT_TYPE') && in_array(WP_ENV, ['prd', 'dev1', 'local'])) {
    Config::define('WP_ENVIRONMENT_TYPE', WP_ENV);
}

// Google reCAPTCHA
Config::define('RECAPTCHA_SITE_KEY', env('RECAPTCHA_SITE_KEY'));
...
Config::define('WPMS_MAIL_FROM_FORCE', env('WPMS_MAIL_FROM_FORCE'));
Config::define('WPMS_MAIL_FROM_NAME', env('WPMS_MAIL_FROM_NAME'));

Config::define('WPMS_MAILER', env('WPMS_MAILER'));
Config::define('WPMS_AMAZONSES_CLIENT_ID', env('WPMS_AMAZONSES_CLIENT_ID'));
Config::define('WPMS_AMAZONSES_CLIENT_SECRET', env('WPMS_AMAZONSES_CLIENT_SECRET'));
Config::define('WPMS_AMAZONSES_REGION', env('WPMS_AMAZONSES_REGION'));
...
Config::define('LOG_TRACE', env('LOG_TRACE'));
...
if (isset($_SERVER['HTTP_X_FORWARDED_PROTO']) && $_SERVER['HTTP_X_FORWARDED_PROTO'] === 'https') {
    ...
    $_SERVER['HTTPS'] = 'on';  
    // If CloudFront header indicates HTTPS, set HTTPS to 'on'
}
...
$config['logger'] = [
    'name' => 'awesome',
    'level' => LOG_LEVEL ? Level::fromName(LOG_LEVEL) : Level::Info,
    'trace' => LOG_TRACE ?: false,
];
...
$stream_handler = new StreamHandler('php://stderr', $config['logger']['level']);
$formatter = new JsonFormatter();
$formatter->includeStacktraces($config['logger']['trace']);
$stream_handler->setFormatter($formatter);

// Push a handler to the logger
